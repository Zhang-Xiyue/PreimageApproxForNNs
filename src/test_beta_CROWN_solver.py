...
class LiRPAConvNet:
    ...
    def build_the_model(self, input_domain, x, data_lb=None, data_ub=None, vnnlib=None, stop_criterion_func=stop_criterion_sum(0), bounding_method=None):
        ...
        approx_tp = preimage_arguments.Config["general"]["approx_tp"]
        ...
        if bounding_method == "alpha-crown":
            ...
            if approx_tp == 'over':
                lb, ub, aux_reference_bounds = self.net.init_slope(
                    (self.x,), share_slopes=share_slopes, c=self.c, bound_lower=False)
            else:
                lb, ub, aux_reference_bounds = self.net.init_slope(
            ...
            if approx_tp == 'over':
                ret = self.net.compute_bounds(x=(x,), C=c_to_use, method='CROWN-Optimized',
                    ...
                    bound_lower=False, bound_upper=True, aux_reference_bounds=aux_reference_bounds, cutter=self.cutter)
            else:
                ret = self.net.compute_bounds(x=(x,), C=c_to_use, method='CROWN-Optimized',
                    return_A=self.return_A, needed_A_dict=self.needed_A_dict,
                    bound_lower=True, bound_upper=False, aux_reference_bounds=aux_reference_bounds, cutter=self.cutter)
            ...
            with torch.no_grad():
                ...
                if self.return_A:
                    ...
                    lb, ub, pre_relu_indices = self.get_candidate(self.net, lb, lb + np.inf)
            # FIXME Note that here return A instead of lA
        ...
        if prune_after_crown_used:
            ...
            prune_after_crown_overhead += time.time() - stime
        if approx_tp == 'over':
            print('initial alpha-CROWN upper bounds:', ub)
            initial_alpha_crown_bounds = ub.max().item()
            print('Best class: (+ rhs)', initial_alpha_crown_bounds)
        else:
            print('initial alpha-CROWN lower bounds:', lb)
        ...
        # return ub[-1], lb[-1], mini_inp, duals, primals, mask, A, lb, ub, pre_relu_indices, slope_opt, history, None
    def copy_alpha(
