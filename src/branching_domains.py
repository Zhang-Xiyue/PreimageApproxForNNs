...
class SortedReLUDomainList(AbstractReLUDomainList):
    ...
    def pick_out(self, batch, device='cpu'):
        ...
        lower_all, upper_all, slopes_all, betas_all, intermediate_betas_all, selected_candidate_domains = [], [], [], [], [], []
        dm_l_all, dm_u_all = [], []
        ...
        while True:
            ...
            # idx2 -= 1
            if selected_candidate_domain.valid is True:
        ...
        if isinstance(selected_candidate_domain, ReLUDomain):
            ...
            #     new_As.append(torch.cat([As[i][j] for i in range(batch)]))
            # # Transfer to GPU.
            # new_As = [t.to(device=device, non_blocking=True) for t in new_As]
        ...
        if isinstance(selected_candidate_domain, ReLUDomain):
            # Relu split domains.
            return new_masks, lower_bounds, upper_bounds, slopes, betas_all, intermediate_betas_all, selected_candidate_domains, cs, thresholds
        else:
            # Input split domains.
