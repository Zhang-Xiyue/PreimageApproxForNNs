...
from test_polyhedron_util import post_process_A, calc_input_coverage_initial, calc_input_coverage_initial_image_under, calc_input_coverage_initial_image_over
...
def load_act_vecs(dataset_tp):
    ...
    elif dataset_tp == "auto_park" or dataset_tp == "Customized_beta_demo_data":
        pre_relu_layer = ['2']
...
def batch_verification(tot_ambi_nodes_sample, mask_sample_ori, score_relus_ori, d, y, net, batch, pre_relu_indices, growth_rate, fix_intermediate_layer_bounds=True,
    ...
    #         branching_decision = [(0,9)]
        if bound_upper and arguments.Config["preimage"]["save_process"]:
            branching_decision = [(1,6)]
            # branching_decision = [(0,4)]
            
        split_depth = 1
    # if arguments.Config["data"]["dataset"] == "auto_park_part":
    #    branching_decision = [(0,0)]
    #    split_depth = 1
    # elif arguments.Config["data"]["dataset"] == "Customized_beta_demo_data":
    #    branching_decision = [(0,0)]
    #    split_depth = 1
...
def initial_check_preimage_approx(A_dict, thre, label):
    ...
    if arguments.Config["preimage"]["under_approx"]:
        ...
        # else:
        #     target_vol, cov_quota = calc_input_coverage_initial(preimage_dict, label)
...
def get_act_vecs(prop_samples, model, dataset_tp):
    if "MNIST" in dataset_tp:
        ...
        pre_relu_layer = ['2']
    elif dataset_tp == "Customized_toy_model":
        pre_relu_layer = ['2', '4']
    elif dataset_tp == "auto_park_part":
        pre_relu_layer = ['2', '4']
    elif "vcas" in dataset_tp:
...
def calc_mask_concrete_samples_from_unstable_idx(x, net, y):
    ...
    # img_idx = arguments.Config["data"]["start"]
...
def relu_bab_parallel_dual(net, domain, x,y, use_neuron_set_strategy=False, refined_lower_bounds=None,
    ...
    if bound_lower:
        while cov_quota < cov_thre:
            ...
            if arguments.Config["preimage"]["save_process"]:
                ...
                save_path = os.path.join(arguments.Config["preimage"]["result_dir"], 'run_example')
                # save_file = os.path.join(save_path,'{}_spec_{}_iter_{}_beta_{}'.format(arguments.Config["data"]["dataset"], arguments.Config["preimage"]["runner_up"], 
                #                                                                        num_iter, arguments.Config["solver"]["beta-crown"]["beta"]))
                # with open(save_file, 'wb') as f:
                #     pickle.dump(preimage_dict_all, f) 
                split_plane_file = os.path.join(save_path, '{}_split_iter_{}_beta'.format(arguments.Config["data"]["dataset"], num_iter))
        ...
        while cov_quota > cov_thre:
            ...
            if arguments.Config["preimage"]["save_process"]:
                ...
                split_plane_list = get_extra_const(net, history_list)
                save_path = os.path.join(arguments.Config["preimage"]["result_dir"], 'run_example')
                save_file = os.path.join(save_path,'{}_spec_{}_iter_{}_relu_dual_over_0_4'.format(arguments.Config["data"]["dataset"], arguments.Config["preimage"]["runner_up"], 
                                                                                       num_iter))
                with open(save_file, 'wb') as f:
                    pickle.dump(preimage_dict_all, f) 
                split_plane_file = os.path.join(save_path, '{}_split_iter_{}_beta_split_plane_1_6'.format(arguments.Config["data"]["dataset"], num_iter))
                with open(split_plane_file, 'wb') as f:
                    pickle.dump(split_plane_list, f)                    
...
def get_extra_const(net, history_list, beta=True):
    # NOTE obtain the intermediate neuron constraints
    split_relu_indices = []
    assert len(history_list) == 2
    if beta:
        for i, layer_split in enumerate(history_list[1]):
