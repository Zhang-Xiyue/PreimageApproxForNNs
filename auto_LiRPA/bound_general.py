...
class BoundedModule(nn.Module):
    ...
    def __init__(self, model, global_input, bound_opts=None,
        ...
        optimize_bound_args = {
            ...
            'enable_beta_crown': True,  # Enable beta split constraint.
    ...
    # NOTE compute symbolic bounding functions for intermediate relus
    def compute_intermediate_relu_bounds(self, node, prior_checked=False):
        if not prior_checked:
            self.check_prior_bounds(node)
        ...
        sparse_C = self.get_sparse_C(
            ...
            ref_intermediate_lb, ref_intermediate_ub)
        newC, reduced_dim, unstable_idx, unstable_size = sparse_C
        # NOTE set the self.needed_A_dict()
        self.A_dict = {}
        self.needed_A_dict = defaultdict(set)
        self.needed_A_dict[node.name].add(self.input_name[0])
        # Compute backward bounds only when there are unstable
        # neurons, or when we don't know which neurons are unstable.
        if unstable_idx is None or unstable_size > 0:
            assert self.return_A
            _, _, A = self.backward_general(
                C=newC, node=node, need_A_only=True, unstable_idx=unstable_idx,
                unstable_size=unstable_size)
    ...
    from .test_optimized_bounds import get_optimized_bounds, init_slope
