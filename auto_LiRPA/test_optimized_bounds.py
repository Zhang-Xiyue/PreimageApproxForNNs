        ...
        for node in relus:
            if enable_opt_interm_bounds and node.sparse_beta is not None:
                ...
                if node.sparse_beta is not None:
                    ...
                    #     betas.append(torch.exp(node.sparse_beta))
                    # else:
...
def get_optimized_bounds(
    ...
    # print("NOTE: we are optimizing the difference between lower and upper bound")
    ...
    for i in range(iteration):
        ...
        if opt_relu_poly or opt_poly_vol:
            if bound_lower:
                loss_ = ret[3]
            else:
                loss_ = -ret[4]
        ...
        # loss_ = l-u
        # FIXME using the upper and lower bounding function
    ...
    if verbosity > 0:
        ...
        if best_ret_u is not None:
            # FIXME: unify the handling of l and u.
            print(
                'best_u after optimization:',
                best_ret_u.sum().item(), 'with beta sum per layer:',
                [p.sum().item() for p in betas])
