...
def input_split_feature_edge(dm_l_all, dm_u_all, shape=None,
                         cs=None, thresholds=None, split_depth=1, i_idx=None):
    """
    Split the dm_l_all and dm_u_all on input idx with longest edge length for all selected subdomains with split depth as 1.
    """
    dm_l_all = dm_l_all.flatten(1)
    dm_u_all = dm_u_all.flatten(1)
...
def input_split_all_feature_parallel(dm_l_all, dm_u_all, shape=None,
                         cs=None, thresholds=None, split_depth=1, i_idx=None):
    """
    Split the dm_l_all and dm_u_all on every possible input idx for all selected subdomains with split depth as 1.
    """
    dm_l_all = dm_l_all.flatten(1)
    dm_u_all = dm_u_all.flatten(1)
